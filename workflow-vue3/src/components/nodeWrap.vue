<template>
  <div class="node-wrap" v-if="nodeConfig.type == 'start' || nodeConfig.type == 'approver' || nodeConfig.type == 'notifier' || nodeConfig.type < 3 ">
    <div class="node-wrap-box" :class="(nodeConfig.type == 'start' ? 'start-node ' : '') +(isTried && nodeConfig.error ? 'active error' : '')">
        <div class="title" :style="`background: rgb(${bgColors[ nodeConfig.type == 'approver' ? 1 : (nodeConfig.type == 'notifier' ? 2 : 0) ]});`">
          <span v-if="nodeConfig.type == 'start'">{{ nodeConfig.name }}</span>
          <template v-else>
            <span class="iconfont">{{nodeConfig.type == 'approver'?'':''}}</span>
            <input
              v-if="isInput"
              type="text"
    ...
    <addNode v-model:childNodeP="nodeConfig.childNode" :nodeConfig="nodeConfig" />
  </div>
        ...
        <div class="col-box" v-for="(item, index) in nodeConfig.conditionNodes" :key="index">
          <div class="condition-node">
            <div class="condition-node-box">
              ...
              <addNode v-model:childNodeP="item.childNode" :nodeConfig="nodeConfig" />
            </div>
          </div>
